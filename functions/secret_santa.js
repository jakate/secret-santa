...
module.exports = function (friendList) {
  ...
  function getList() {
    ...
    var results = _.map(people, (person) => {
  ...
  while (results === null) {
    ...
    people.forEach((person) => {
      var isInList = _.filter(results, (r) => { return r.target.name === person.name })
    ...
    people.forEach((person) => {
      var isInList = _.filter(results, (r) => { return r.name === person.name })
      if (isInList.length !== 1) {
